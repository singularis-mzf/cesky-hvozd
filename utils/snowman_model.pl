...
sub get_v # (x, y, z)
{
	...
	#my $text = sprintf("v %.6f %.6f %.6f # %.6f %.6f %.6f # %.6f %.6f %.6f", $x, $y, $z, $v_scale[0], $v_scale[1], $v_scale[2], $v_shift[0], $v_shift[1], $v_shift[2]);
...
sub nodebox
{
	...
	# right (+x):
	ctverec(4, [$z_min + 0.5, $z_max + 0.5], [$y_min + 0.5, $y_max + 0.5], $x_max, [$y_min, $y_max], [$z_min, $z_max], 1, 1, 1);
	# left (-x):
	ctverec(3, [$z_max + 0.5, $z_min + 0.5], [$y_min + 0.5, $y_max + 0.5], $x_min, [$y_min, $y_max], [$z_min, $z_max], -1, 0, 1);
	# back (+z):
...
{
	my $off = 0.25;
	nodebox(-4/16, $off + -8/16, -4/16, 4/16, $off + 2/16, 4/16); # bottom
	nodebox(-3/16, $off + 2/16, -3/16, 3/16, $off + 8/16, 3/16); # head
	nodebox(-8.0/16, $off - 8.0/16, 0.0, 8.0/16, $off + 3.0/16, 0.0); # hands overlay
...
mkdir("out");
open(my $f1, ">:utf8", "out/snowmen.obj") or die("Nelze otevřít soubor!");
$file = $f1;
@v_scale = (2.0, 2.0, 2.0);
fprintf($file, "%s", snowmen_model());
