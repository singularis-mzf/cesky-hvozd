...
function jungle_boat.on_rightclick(self, clicker)
	if not clicker or not clicker:is_player() then
		return
	end
	local name = clicker:get_player_name()
	if self.driver and name == self.driver then
		-- Cleanup happens in jungle_boat.on_detach_child
		clicker:set_detach()
...
function jungle_boat.on_detach_child(self, child)
...
function jungle_boat.on_activate(self, staticdata, dtime_s)
...
function jungle_boat.get_staticdata(self)
	return tostring(self.v)
end


function jungle_boat.on_punch(self, puncher)
	...
	if not self.driver then
		...
		if not minetest.is_creative_enabled(name)
				or not inv:contains_item("main", "more_boats:jungle_boat") then
			local leftover = inv:add_item("main", "more_boats:jungle_boat")
...
function jungle_boat.on_step(self, dtime)
