...
local flak_warning = S("You have entered restricted airspace! You will be shot down in @1 seconds by anti-aircraft guns!", flak_warning_time)
...
if enable_flak then
	minetest.register_chatcommand("area_flak", {
		params = "<ID>",
		description = "Přepne bezletovou zónu pro zónu <ID>",
		func = function(name, param)
			local id = tonumber(param)
			if not id then
				return false, "Chybné použití, viz /help area_flak."
			end
			if not areas:isAreaOwner(id, name) then
				return false, "Oblast "..id.." neexistuje nebo vám nepatří."
			end
			local open = not areas.areas[id].flak
			-- Save false as nil to avoid inflating the DB.
			areas.areas[id].flak = open or nil
			areas:save()
			return true, "Zóna "..id..": "..(open and "bezletová" or "letecký provoz povolen")
		end
	})
end
