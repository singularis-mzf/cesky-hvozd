...
local function wear_out(player_name, item, n_steps, size)
    ...
    if item:is_empty() then
        return ItemStack(shared.spray_can_empty)
    end
    return item
end
...
local function spray_can_on_place(item, player)
	if item:get_name() == shared.spray_can_rgb then
		shared.show_gui(player)
		return
	end

    local meta = item:get_meta()
	local current_size = shared.meta_get_size(meta)
	local new_size
	if current_size < 3 then
		new_size = current_size + 1
	else
		new_size = 1
	end
	shared.meta_set_size(item, meta, new_size)
	return item
end
...
for _, dye in ipairs(shared.game_dyes) do
    local item_name = shared.spray_can_prefix .. dye.name
    ...
    minetest.register_tool(item_name, {
		...
		_ch_help = "Barevný sprej:\nLevým klikem kreslí, pravým klikem se přepíná velikosti stopy (1, 2, 3).\n"..
            "Prázdný sprej budete moci recyklovat roztavením na železné ingoty. Malby sprejem nepřežijí restart serveru!",
...
minetest.register_craft({
    ...
    output = shared.spray_can_rgb,
})

minetest.register_craftitem(shared.mushroom_red_extract, {
...
minetest.register_craft({
    ...
    output = shared.mushroom_red_extract.." " .. shared.game_items.red_mushroom_extract_count,
})

minetest.register_tool(shared.spray_can_remover, {
...
minetest.register_craft({
    recipe = {
        { shared.game_items.iron_ingot     },
        { shared.mushroom_red_extract },
        { shared.game_items.iron_ingot     },
    },
    output = shared.spray_can_remover,
})
