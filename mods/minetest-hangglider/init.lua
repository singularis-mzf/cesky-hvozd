...
if core.global_exists("areas") then
	...
	minetest.register_chatcommand("area_flak",{
		...
		func = function(name, param)
			...
			areas:save()
			return true, S("Area's airspace @1.", open and S("closed") or S("opened"))
		end
	})
end
...
hangglider.can_fly = function (pname, pos)
	...
	if core.global_exists("wardzones") then
	...
	if core.global_exists("areas") and minetest.is_protected(vector.round(pos), pname) then
			...
			if player then
				...
				if hangglider.use[pname] then
					...
					if mrn_name then
						if not (mrn_name.walkable or mrn_name.liquidtype ~= "none") then
							...
							if not core.global_exists("minetestd") then
				...
				if not hangglider.can_fly(pname,pos) then
				    if not self.warned then -- warning shot
						...
						minetest.chat_send_player(pname, S("Protected area! You will be shot down in two seconds by anti-aircraft guns!"))
				    end
