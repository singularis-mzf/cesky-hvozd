    ...
    get_formspec = function(player, perplayer_formspec)
        ...
        formspec = formspec ..
            ("image[%f,%f;%f,%f;ui_crafting_long_arrow.png]"):format(
                ...
                ui.imgscale,
                ui.imgscale * 3) ..
            ("button[%f,%f;%f,%f;craft_craftall;%s]tooltip[craft_craftall;%s]"):format(
                perplayer_formspec.craft_arrow_x,
                perplayer_formspec.craft_y + 1.50,
                perplayer_formspec.btn_size * 2,
                perplayer_formspec.btn_size,
                F(S("vyrobit\nvše")),
                F("Provede výrobu z výrobní mřížky opakovaně; tolikrát, kolikrát to jen půjde.")
            )
...
local function craft_craftall(player)
    ...
    if (
        ...
        ch_core.systemovy_kanal(player_name, S("You are too hungry to use Craft All at this time."))
        return
    end ]]
    ...
    while true do
        ...
        if has.stamina and stamina.exhaust_player_by_craft then
			stamina.exhaust_player_by_craft(player)
        end
		ch_core.ap_announce_craft(player)
