...
local tooltip = "Přeuspořádá předměty ve výrobní mřížce podle vzoru."
    ...
    get_formspec = function(player, perplayer_formspec)
        ...
        if perplayer_formspec.pagecols == 4 then
            ...
            for i, v in ipairs(uip.craft_patterns) do
                formspec = formspec ..
                    ("image_button[%f,%f;%f,%f;%s;craft_organize_%i;]tooltip[craft_organize_%i;%s]"):format(
...
local function craft_organize(player, fields)
    ...
    if not pattern_id or not uip.craft_patterns[pattern_id] then
		ch_core.systemovy_kanal(player_name, "Nečekaný vzor!?")
        return
    end
    ...
    if not single_item then
        -- craft inv is empty
		ch_core.systemovy_kanal(player_name, "CHYBA: Výrobní mřížka je prázdná.")
        return
    end

    if not all_identical(craft_list) then
		ch_core.systemovy_kanal(player_name, "CHYBA: Pro reorganizaci musejí být všechny předměty ve výrobní mřížce téhož typu.")
        return
    end
    ...
    if not def then
		ch_core.systemovy_kanal(player_name, "CHYBA: Nelze reorganizovat neznámý předmět.")
        return
    end
    ...
    if num_items > stacksize * pattern_size then
		ch_core.systemovy_kanal(player_name, "CHYBA: Příliš mnoho předmětů na požadované uspořádání.")
        return
    end

    if num_items < pattern_size then
        ch_core.systemovy_kanal(player_name, "CHYBA: Nedostatek předmětů.")
        return
    end
