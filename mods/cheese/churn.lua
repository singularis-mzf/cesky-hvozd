...
if cheese.there_is_milk then
	...
	if minetest.registered_items["mobs:butter"] then
		minetest.unregister_item("mobs:butter")
		minetest.register_alias("mobs:butter", "cheese:butter")
	end
	if minetest.registered_items["mobs:bucket_wooden_milk"] then
		table.insert(churnable, {"mobs:bucket_wooden_milk", "cheese:butter"})
	end
...
local function should_return (item_name) -- only inside the is_accettable_source check therefor there is no need for the food_vegan check
	if item_name == "mobs:bucket_wooden_milk" then
		return "bucket_wooden:bucket_empty"
