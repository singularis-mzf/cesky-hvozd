...
worldedit.register_command("outset", {
	params = "[h/v] <metrů>",
	description = "Zvětší výběr vodorovně či svisle.",
	...
	parse = function(param)
		...
		if hv_test ~= nil then
			return false, S("Invalid direction.")
		end
...
worldedit.register_command("inset", {
	params = "[h/v] <amount>",
	description = "Zmenší výběr vodorovně či svisle.",
	privs = {worldedit=true},
	require_pos = 2,
	parse = function(param)
		local find, _, dir, amount = param:find("(%a*)%s*([+-]?%d+)")
...
worldedit.register_command("shift", {
	...
	description = "Posune výběr bez přesunu jeho obsahu",
	privs = {worldedit=true},
	require_pos = 2,
	parse = function(param)
...
worldedit.register_command("expand", {
	...
	description = "Rozšíří výběr podle dané osy",
	privs = {worldedit=true},
	require_pos = 2,
	parse = function(param)
...
worldedit.register_command("contract", {
	params = "[+/-]x/y/z/?/up/down/left/right/front/back <amount> [reverse amount]",
	description = "Stlačí výběr podle zadané osy",
	privs = {worldedit=true},
	require_pos = 2,
	parse = function(param)
		local find, _, sign, direction, amount,
