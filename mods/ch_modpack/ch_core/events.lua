ch_core.open_submod("events", {lib = true})
...
local events = function()
    ...
    for i = 1, 3 do
        ...
        result[i] = data or {}
        print("DEBUG: loaded "..#result[i].." events for "..month_id)
        if mesic > 1 then
            mesic = mesic - 1
        else
            mesic, rok = 12, rok - 1
...
function ch_core.add_event(event_type, text, player_name)
            ...
            if current_events[i].type == event_type then
                ...
                if count >= safety_limit then
                    minetest.log("warning", "Event NOT added, because the security limit "..safety_limit.." was achieved: "..
                        timestamp.." <"..event_type.."> "..(get_event_text(event) or "nil"))
                    return
                end
            end
        end
    ...
    save_events()
    minetest.log("action", "Event #"..#current_events.." added: "..timestamp.." <"..event_type.."> "..(get_event_text(event) or "nil"))
end
...
function ch_core.get_events_for_player(player_name_or_player, allowed_event_types, limit)
    ...
    for _, events_month in ipairs(events) do
        for i = #events_month, 1, -1 do
            ...
            if type_def ~= nil and (event.type ~= "player_only" or event.player_name == pinfo.player_name) then
                local record = {
...
ch_core.register_event_type("server_started", {
    ...
    access = "public", -- později změnit na "admin"
    default_text = "",
})
