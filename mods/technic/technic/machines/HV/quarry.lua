...
local function set_quarry_formspec(meta)
	...
	formspec = formspec.."label[0,4;"..minetest.formspec_escape(
			...
			(rel_y < -quarry_max_depth and S("Digging finished") or
				...
				S("Digging @1 m "..(rel_y > 0 and "above" or "below").." machine", string.format("%d", math.abs(rel_y)))))
			).."]"
...
local function quarry_run(pos, node)
	...
	if meta:get_int("enabled") ~= 0 and meta:get_int("HV_EU_input") >= quarry_demand and meta:get_int("purge_on") == 0 then
