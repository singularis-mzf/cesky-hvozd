...
local function update_meta(meta)
	...
	if channel == "" then
		...
		local desc = S(cr and "sending and receiving" or "sending")
...
local function can_go(pos, node, velocity, stack)
	...
	if channel == "" then
		core.log("warning", "Stack "..stack:get_name().." "..stack:get_count().." refused by Teleporting Tube at "..pos_str.." due to channel not set.")
		return {}
	end
	local receivers = get_receivers(pos, channel)
	if #receivers == 0 then
		core.log("warning", "Stack "..stack:get_name().." "..stack:get_count().." refused by Teleporting Tube at "..pos_str.." due to no receivers found at channel '"..channel.."'")
		return {}
	end
	...
	pos.y = target.y
	pos.z = target.z
	core.log("action", "Stack "..stack:get_name().." "..stack:get_count().." teleported by Teleporting Tube from "..pos_str.." to "..core.pos_to_string(pos).." (selected from "..#receivers.." receivers) via channel '"..channel.."'")
