...
local function get_record_simple(name)
	...
	elseif (not e.record) or (#e.record == 0) then
		return nil, ("`%s' nemá žádné záznamy o vyhoštění."):format(name)
	end
	local record = { }
	for _, rec in ipairs(e.record) do
		local msg = (os.date("%Y-%m-%d %H:%M:%S", rec.time).." | "
				..(rec.reason or "Důvod neuveden."))
...
local function make_fs(name)
	...
	local fs = {
		...
		"button[14,-.3;2,1;search_submit;Hledat]",
	}
	local fsn = #fs
	...
	if record_name then
		...
		if record then
			...
			if rec then
				fsn=fsn+1 fs[fsn] = format("label[0,10.3;%s]",
						ESC("Zdroj: "..(rec.source or "<žádný>")
							.."\nČas: "..os.date("%c", rec.time)
							.."\n"..(rec.expires and
		...
		local e = "Žádná položka nenalezena."
		fsn=fsn+1 fs[fsn] = "textlist[4.2,.8;11.7,9.3;err;"..ESC(e)..";0]"
		fsn=fsn+1 fs[fsn] = "label[0,10.3;"..ESC(e).."]"
	end
...
minetest.register_chatcommand("xban_gui", {
	description = "Zobrazit grafické rozhraní XBanu",
	params = "",
	privs = { ban=true, },
