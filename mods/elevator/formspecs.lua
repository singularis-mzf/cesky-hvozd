...
minetest.register_on_player_receive_fields(function(sender, formname, fields)
    ...
    if fields.edit then
        ...
        minetest.show_formspec(sender:get_player_name(), formname, formspec)
        return true
    end
    ...
    if fields.target then
        ...
        if (not (pi.major == 0 and pi.minor == 4 and pi.patch == 15)) and (pi.protocol_version or 29) < 29 then
            closeformspec = "size[4,2] label[0,0;Nyní používáte výtah\nPovyšte Minetest, abyste se zbavili tohoto dialogu.] button_exit[0,1;4,1;close;Close]"
        end
        -- End hacky HACK.
        ...
        if not motor then
            ...
            if motor then
                ...
                minetest.chat_send_player(sender:get_player_name(), "Překalibrováno na nový motor, prosím zkuste to znovu.")
                return true
            end
        end
        if not motor then
            minetest.chat_send_player(sender:get_player_name(), "Tomuto výtahu chybí motor.")
            return true
        end
        ...
        if target then
            ...
            if elevator.boxes[motorhash] then
                minetest.chat_send_player(sender:get_player_name(), "Výtah je již obsazen.")
                return true
            end
            ...
            minetest.chat_send_player(sender:get_player_name(), "Cíl je neplatný.")
            return true
        end
        return true
    end
