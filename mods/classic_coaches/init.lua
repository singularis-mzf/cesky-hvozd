...
local function recipe_to_drops(recipe)
	local set = {}
	for _, row in ipairs(recipe) do
		for _, s in ipairs(recipe) do
			if core.registered_items[s or "~~~"] then
				set[s] = (set[s] or 0) + 1
			end
		end
	end
	local result = {}
	for name, count in pairs(set) do
		if count == 1 then
...
for _, wagon in pairs(wagons) do
	advtrains.register_wagon(wagon.wagon_type, {
		...
		drops= recipe_to_drops(wagon.recipe),
