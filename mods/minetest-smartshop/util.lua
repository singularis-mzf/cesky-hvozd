...
function util.check_shop_add_remainder(shop, remainder)
	...
	util.error("CHYBA: obchodní terminál postavy %s na pozici %s ztratil %q při přidávání", owner, pos_as_string, remainder:to_string())
...
function util.check_shop_remove_remainder(shop, remainder, expected)
	...
	util.error("CHYBA: obchodní terminál postavy %s na pozici %s ztratil %q z %q při odebírání",
...
function util.check_player_add_remainder(player_inv, shop, remainder)
	...
	util.error("CHYBA: %s ztrail/a %q při přidávání, používajíc obchodní terminál od %s na pozici %s",
...
function util.check_player_remove_remainder(player_inv, shop, remainder, expected)
	...
	util.error("CHYBA: %s ztratil/a %q z %q při odebírání z obchodního terminálu postavy %s na pozici %s",
...
function util.clone_tmp_inventory(inv_name, src_inv)
	...
	for name, _ in pairs(src_inv:get_lists()) do
		if not tmp_inv:is_empty(name) or tmp_inv:get_size(name) ~= 0 then
			util.error("pokus o znovupoužití existujícího dočasného inventáře %s", inv_name)
			return
		end
	end
