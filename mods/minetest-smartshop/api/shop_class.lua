...
function shop_class:log_purchase(player, i, mechanic)
    ...
    smartshop.log("action", "%s koupil/a %q za %q z obchodního terminálu postavy %s na pozici %s přes %s",
...
function shop_class:update_info()
    ...
    if #info_lines == 0 then
        ...
        if variant == "smartshop:shop_full" then
            table.insert(lines, S("Tento terminál je přeplněný."))

        elseif variant == "smartshop:shop_empty" then
            table.insert(lines, S("Tento terminál je vyprodaný."))

        else
            table.insert(lines, S("Tento terminál není nastavený."))
        end
